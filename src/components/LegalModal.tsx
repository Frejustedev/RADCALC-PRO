    ...
    privacy: {
      ...
      body: (
        <div className="space-y-4 text-sm text-slate-300">
          ...
          <p>Les données sont stockées dans <strong>Cloud Firestore (Google Firebase)</strong>, <strong>sous-traitant</strong> du centre (contrat de traitement des données Google / DPA applicable), chiffrées en transit (TLS) et au repos côté serveur. L'accès est protégé par une authentification obligatoire et des règles de sécurité qui le restreignent <strong>au seul personnel autorisé du centre</strong>, selon son rôle (médecin, radiopharmacien, manipulateur…). Un cache local du navigateur (IndexedDB, non chiffré au niveau applicatif) permet le fonctionnement hors-ligne, avec synchronisation au retour du réseau — à protéger par le verrouillage de session du poste.</p>
          <p className="text-xs text-slate-400"><strong>Résidence des données</strong> : la région d'hébergement Firestore est définie par le centre lors de la configuration (une région UE est recommandée). <strong>Durée de conservation</strong> et <strong>droits des personnes</strong> (accès, rectification, effacement) : définis par le centre, responsable de traitement.</p>
